...
if [ "$DROPDB" == "yes" ]
        then
        mongo mna-db.uat.opentestsystem.org/mna --eval "db.dropDatabase()"
fi
...
if [ -f $CATALINA_PATH/webapps/ROOT.war ]
        then
                echo "old versions war file is existed"
        else
...
if [ $? == 0 ]
	then 
		echo "this is monitoring-alerting.rest-VERSION.war"
...
if [ $? == 0 ]
	then 
		echo "this is monitoring-alerting.webapp-VERSION.war"
...
echo $ARTIFACT | grep jmx

if [ $? == 0 ]
	then 
		echo "this is monitoring-alerting.jmx-VERSION.war"
		cp -r $CATALINA_PATH/webapps/mna-jmx.war $Backuppath/
		
		rm -r $CATALINA_PATH/webapps/mna-jmx.war
		rm -r $CATALINA_PATH/webapps/mna-jmx
		
		mv $Latestpath/$ARTIFACT-$Version.war $Latestpath/mna-jmx.war
		cp -r $Latestpath/mna-jmx.war $CATALINA_PATH/webapps/
