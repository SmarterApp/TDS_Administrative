...
if [ "$DROPDB" == "yes" ]
        then
        mongo tsb-db.uat.opentestsystem.org/tsb --eval "db.dropDatabase()"
fi
...
if [ $? == 0 ]
	then 
		echo "this is test-spec-bank.rest-VERSION.war"
...
if [ $? == 0 ]
	then 
		echo "this is test-spec-bank.webapp-VERSION.war"
